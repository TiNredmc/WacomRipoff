...
uint8_t hid_report[7];// store HID report for sending via usb

void wacom_query_device(I2C_HandleTypeDef *hi2c, struct wacom_features *features)
{

	uint8_t cmd1[] = { WACOM_CMD_QUERY0, WACOM_CMD_QUERY1,
			WACOM_CMD_QUERY2, WACOM_CMD_QUERY3 };
	uint8_t cmd2[] = { WACOM_CMD_THROW0, WACOM_CMD_THROW1 };
...
//	unsigned char tsw, f1, f2, ers;
	...
	HAL_I2C_Master_Receive(wacom_i2c.hi2c, WACOM_ADDR, data, sizeof(data), 150);
	HAL_GPIO_WritePin(wacom_i2c.GPIOx, wacom_i2c.irq, GPIO_PIN_SET);
...
/*
	...
	hid_report[1] = data[4];// Send lower byte first
	hid_report[2] = data[5];// Then send higher byte

    // hid_report[3] and hid_report[4] is for Y position
	hid_report[3] = data[6];// Send lower byte first
	hid_report[4] = data[7];// Then send higher byte

    // hid_report[5] and hid_report[6] is for pressure
	hid_report[5] = data[8];// Send lower byte first
	hid_report[6] = data[9];// Then send higher byte
...
// TODO : if the array copy like that is sending correct HID data, Then use memcpy instead
	...
	// DONE
	HAL_GPIO_WritePin(wacom_i2c.GPIOx, wacom_i2c.irq, GPIO_PIN_RESET);
}


void wacom_i2c_probe(I2C_HandleTypeDef *hi2c, GPIO_TypeDef* GPIOx, uint16_t RST_pin, uint16_t IRQ_stat_pin)
{
	struct wacom_features features = { 0 };

	wacom_i2c.hi2c = hi2c;// Get the typedef of I2C
	wacom_i2c.GPIOx = GPIOx;// Get the GPIO port
