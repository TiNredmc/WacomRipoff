...
uint8_t hid_report[7]={0,0,0,0,0,0,0};// store HID report for sending via usb
...
/*
	...
	hid_report[2] = data[7];// Then send higher byte

    // hid_report[3] and hid_report[4] is for Y position
	hid_report[3] = data[4];// Send lower byte first
	hid_report[4] = data[5];// Then send higher byte
